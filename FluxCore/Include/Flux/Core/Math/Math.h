...
namespace Flux {
    ...
    public:
        ...
        template<typename NumberType>
        FORCEINLINE static NumberType easeIn(NumberType const& from, NumberType const& to, float t, float easePower = 2.0f) {
            return lerp<NumberType>(from, to, powf(t, easePower));
        }
        
        template<typename NumberType>
        FORCEINLINE static NumberType easeOut(NumberType const& from, NumberType const& to, float t, float easePower = 2.0f) {
            return lerp<NumberType>(from, to, flip(powf(flip(t), easePower)));
        }
        
        template<typename NumberType>
        FORCEINLINE static NumberType easeInOut(NumberType const& from, NumberType const& to, float t, float easePower = 2.0f) {
