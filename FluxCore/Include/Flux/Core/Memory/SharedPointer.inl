...
#include <Flux/Core/Memory/WeakPointer.h>
...
namespace Flux {
    ...
    template <typename T>
    SharedPointer<T>::SharedPointer(nullptr_t) noexcept { }
    ...
    template <typename T>
    template <typename Y>
    bool SharedPointer<T>::operator==(SharedPointer<Y> const& other) const noexcept { return pointer == other.pointer; }
    ...
    SharedPointer<T>::operator T*() {
        fassertf(pointer, "Tried to dereference a null pointer.");
        return pointer;
    }
    ...
    template <typename T>
    bool SharedPointer<T>::valid() const { return pointer; }
