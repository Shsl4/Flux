...
namespace Flux {
    ...
    template <typename T>
    template <typename Y>
    bool WeakPointer<T>::operator==(WeakPointer<Y> const& other) const noexcept { return pointer == other.pointer; }
    ...
    template <typename T>
    SharedPointer<T> WeakPointer<T>::retain() const { return SharedPointer<T>(*this); }
    ...
    bool WeakPointer<T>::expired() const {
        ...
        if (counter->exhausted()) { return true; }
    ...
    template <typename T>
    bool WeakPointer<T>::valid() const { return !expired(); }

    template <typename T>
    T* WeakPointer<T>::operator->() const {
        fassertf(valid(), "Tried to dereference a null pointer.");
        return pointer;
    }
